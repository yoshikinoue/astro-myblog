...
## 2025-05-26 - Naive CMS Path Sanitization
**Vulnerability:** Path Traversal in `tina/config.ts` via insecure custom slugify
**Learning:** Returning `slug()` raw input from an untrusted source can allow path traversal during build time since `github-slugger` does not fully strip all path separator variations.
**Prevention:** Always implement an explicit regex to strip forward and back slashes `[\\/]` and dot sequences `\.+` before calling `slug()` on file paths generated by CMS content.
