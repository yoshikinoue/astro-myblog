...
## 2025-05-26 - Structural XSS Vulnerability via set:html
**Vulnerability:** Rendering raw HTML from strings using `set:html` based on dynamic keys (e.g., `socialIcons[social.name]`) created a structural XSS vulnerability.
**Learning:** Even when the dictionary is currently trusted, dynamic HTML rendering is risky. Using Astro static template elements is a safer pattern.
**Prevention:** Render SVGs as static template elements within a dedicated component (e.g., `SocialIcon.astro`) to eliminate the risk of HTML injection.
