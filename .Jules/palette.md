...
## 2025-04-17 - Disabled Link UX and Accessibility
**Learning:** When replacing `pointer-events: none` with `cursor-not-allowed` on disabled anchor (`<a>`) elements to enable visual feedback, it is critical to also completely remove the `href` attribute (e.g., passing `href={undefined}` in Astro). Passing `undefined` omits the attribute completely, whereas leaving a dummy `href` like `#` keeps the element fully focusable and clickable (functionally flawed) despite the cursor style.
**Action:** Always combine `cursor-not-allowed` with `href={undefined}` on disabled anchor tags to ensure both proper visual indication and strict un-clickability/un-focusability for semantic and functional safety.
## 2025-04-17 - Disabled Link UX and Accessibility
**Learning:** When replacing `pointer-events: none` with `cursor-not-allowed` on disabled anchor (`<a>`) elements to enable visual feedback, it is critical to also completely remove the `href` attribute (e.g., passing `href={undefined}` in Astro). Passing `undefined` omits the attribute completely, whereas leaving a dummy `href` like `#` keeps the element fully focusable and clickable (functionally flawed) despite the cursor style.
**Action:** Always combine `cursor-not-allowed` with `href={undefined}` on disabled anchor tags to ensure both proper visual indication and strict un-clickability/un-focusability for semantic and functional safety.
