...
## 2026-03-30 - Disabled links lack explanatory tooltips and jump to top of page
**Learning:** Using `pointer-events-none` on disabled UI elements blocks pointer events completely, preventing native browser tooltips (`title` attribute) from appearing and explaining *why* an action is disabled. Additionally, providing `href="#"` for a disabled link triggers a confusing scroll-to-top behavior if somehow clicked. In Astro JSX, passing `href={undefined}` correctly drops the attribute entirely.
**Action:** When styling disabled elements, use `cursor-not-allowed` rather than `pointer-events-none` so tooltips remain accessible. For disabled `<a>` links, conditionally pass `href={undefined}` instead of a placeholder `#` to render it semantically inert.
