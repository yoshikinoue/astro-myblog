...
**Learning:** Using `pointer-events-none` and `href="#"` prevents native tooltips and causes scroll jumps on disabled links.
**Action:** Use `cursor-not-allowed` and omit the href attribute entirely (`href={undefined}`) to provide accessible visual feedback and prevent unintended behavior on disabled links.
