...
## 2026-02-18 - Cursor and Tooltip feedback on disabled Pagination links
**Learning:** By setting `href={undefined}` instead of `href="#"` for Astro `<a/>` links, they gracefully degrade into non-interactive elements without losing structural integrity or accidentally falling into the keyboard tab order. We can then remove `pointer-events-none` from `.disabled` pagination components and replace it with `cursor-not-allowed` and a helpful `title`. This enables users to see a descriptive tooltip explaining why the link is disabled (e.g., "No previous page").
**Action:** When creating disabled link components via anchor (`<a>`) tags, remove the `href` attribute completely (`href={undefined}`) and apply `cursor-not-allowed`. This allows native `title` tooltips and intuitive cursors to display while fully satisfying keyboard and screen reader accessibility regressions.
