...
## 2025-03-25 - Synchronized Hover States on Complex Buttons
**Learning:** When a button contains both an SVG icon and text (like the "Go back" button), applying a simple `hover:opacity-75` can feel disconnected if the elements don't share a strong color identity. Using Tailwind's `group` on the parent and `group-hover` on the child SVG to synchronize a color change (e.g., to the theme's accent color) creates a much more cohesive and engaging interactive cue.
**Action:** Always check multi-element interactive components for synchronized hover states. Use `group` and `group-hover` (or `fill="currentColor"`) to ensure the entire click target responds visually as a single unit, reinforcing its interactivity.
