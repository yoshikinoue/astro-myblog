...
## 2024-04-30 - Improve disabled link accessibility
**Learning:** Using `pointer-events-none` on disabled elements prevents native tooltips (like the `title` attribute) from showing up. Replacing `pointer-events-none` with `cursor-not-allowed` on anchor elements provides visual feedback of the disabled state, but it requires removing the `href` attribute (using `href={undefined}`) to ensure the element is semantically disabled, not keyboard focusable, and visually unclickable, while still allowing tooltips to appear on hover.
**Action:** When designing disabled link states, prefer `cursor-not-allowed` over `pointer-events-none` so tooltips remain functional. Ensure `href={undefined}` is passed to Astro elements to semantically detach the link behavior.
