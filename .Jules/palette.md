...
## 2026-02-18 - Tooltips on disabled buttons using cursor-not-allowed
**Learning:** Using `pointer-events-none` on disabled elements prevents hover states, including native tooltips (`title`), causing confusion as to why the element is disabled.
**Action:** When styling disabled elements, use `cursor-not-allowed` to indicate the disabled state while allowing `title` tooltips to function. Ensure `href` is fully removed (`href={undefined}`) to preserve keyboard accessibility (preventing focus).
