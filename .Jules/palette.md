...
## 2026-02-18 - Semantic Disabled Links
**Learning:** Using `href="#"` and `pointer-events-none` for disabled links prevents navigation but removes native visual feedback (cursors, tooltips) and causes accidental scroll-to-top behavior if clicked.
**Action:** Always omit the `href` attribute entirely (`href={undefined}`) for semantically disabled links. Replace `pointer-events-none` with `cursor-not-allowed` to provide clear, native visual feedback that the action is unavailable without breaking accessibility.
