...
## 2024-05-12 - Disabled tooltips and pointer-events
**Learning:** Using `pointer-events: none` on disabled anchor elements prevents native `title` tooltips from appearing and intercepts all mouse events. To maintain the non-interactive visual state while allowing tooltips, use `cursor-not-allowed`.
**Action:** When styling disabled anchor elements to support tooltips, use `cursor-not-allowed` instead of `pointer-events-none`. Additionally, ensure the `href` attribute is removed (e.g., `href={undefined}`) to prevent the element from being keyboard focusable or clickable.
