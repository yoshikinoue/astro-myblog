...
**Learning:** Using `pointer-events: none` on disabled pagination anchor (`<a>`) elements prevents browser-native tooltips (`title` attribute) from displaying, leaving users without an explanation of why the action is disabled.
**Action:** When styling disabled anchors to support tooltips, use `cursor-not-allowed` instead of `pointer-events: none`, and ensure the `href` attribute is completely omitted (e.g., `href={undefined}`) to prevent the element from being keyboard focusable or clickable.
