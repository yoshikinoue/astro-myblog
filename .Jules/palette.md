...
## 2026-02-18 - Tooltips on disabled buttons
**Learning:** Using `pointer-events: none` on disabled anchor tags hides native title tooltips because it prevents all pointer events, leaving users without context on why an action is disabled.
**Action:** When styling disabled links, use `cursor-not-allowed` and set `href={undefined}`. This removes focusability and clickability natively, while preserving pointer events so that `title` or tooltip attributes remain functional on hover.
