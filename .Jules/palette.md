...
## 2026-05-17 - Improve disabled link states
**Learning:** Using `pointer-events: none` on disabled elements prevents mouse events, removing the visual feedback of a disabled cursor. Replacing it with `cursor-not-allowed` restores this feedback. Furthermore, native `<a>` elements used as buttons should have their `href` attribute entirely removed when disabled, natively dropping them from the accessibility tree and keyboard focus order without relying solely on `tabindex="-1"`.
**Action:** When creating disabled links, use `cursor-not-allowed` for visual feedback and omit the `href` attribute completely (e.g., `href={disabled ? undefined : href}`) to prevent accessibility regressions.
