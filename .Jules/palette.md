...
## 2026-02-18 - Tooltips on disabled pagination links
**Learning:** Disabled pagination links used `pointer-events-none`, which prevented users from seeing tooltips explaining why they couldn't go back or forward. Removing `href` fully disables the `<a>` tag while allowing pointer events for tooltips.
**Action:** When disabling links, set `href={undefined}` to remove the attribute, use `cursor-not-allowed` instead of `pointer-events: none`, and provide a helpful `title` to explain the disabled state.
