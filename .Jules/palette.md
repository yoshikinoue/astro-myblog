...
## 2024-05-28 - Tooltips for disabled Anchor pagination buttons
**Learning:** In Astro, when styling disabled anchor (`<a>`) elements with `cursor-not-allowed` instead of `pointer-events: none` to support native `title` tooltips, the `href` attribute must be set to `undefined` (which completely removes it from the rendered HTML) instead of `#`. This prevents the element from being keyboard focusable or clickable, avoiding accessibility regressions.
**Action:** When adding tooltips to disabled links, swap `pointer-events-none` for `cursor-not-allowed` and always use `href={undefined}` to maintain keyboard accessibility.
