...
## 2026-02-18 - Missing tooltips on disabled interactive elements
**Learning:** Pagination buttons in `Posts.astro` lacked tooltips explaining why they were disabled, leaving users wondering if the app was broken or if there were simply no more pages. Furthermore, the `pointer-events-none` class prevented tooltips (`title` attributes) from showing up at all on hover.
**Action:** Replace `pointer-events-none` with `cursor-not-allowed` on disabled links to allow hover events, and explicitly remove the `href` attribute (e.g., `href={disabled ? undefined : href}`) to securely disable link functionality while still displaying informative tooltips explaining the state (e.g., "No previous pages").
