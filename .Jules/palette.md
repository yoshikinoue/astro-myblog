...
## 2026-02-18 - Missing title attribute on icon-only Back to Top button
**Learning:** While the Back to Top button had an aria-label for screen readers, it lacked a title attribute, leaving sighted mouse users without a native tooltip to explain the icon's function on hover.
**Action:** Always pair aria-label with a matching title attribute on icon-only buttons to ensure both assistive technology users and sighted mouse users receive descriptive text.
