...
## 2024-05-24 - Improve Disabled Button UX with cursor-not-allowed
**Learning:** Using `pointer-events: none` on disabled anchor links hides them from pointer interactions, preventing tooltips or natural disabled cursors from appearing. Switching to `cursor-not-allowed` while removing the `href` attribute (via `undefined`) restores the disabled cursor UI feedback while fully preventing click navigation and keyboard focus, resulting in a significantly better UX without accessibility regressions.
**Action:** When styling disabled links, prefer `cursor-not-allowed` and removing the `href` over `pointer-events: none` to support visual feedback states like tooltips and default cursors.
