...
## 2026-02-18 - Tooltips and interaction on disabled links
**Learning:** Using `pointer-events: none` on disabled links prevents native tooltips (`title` attributes) from appearing and hides the disabled state from pointer interaction (like showing a `not-allowed` cursor). Furthermore, setting `href="#"` on disabled links keeps them semantically interactive, creating confusion.
**Action:** When styling disabled links, remove the `href` attribute entirely (`href={undefined}`) to correctly disable the link behavior, and use `cursor-not-allowed` instead of `pointer-events: none` so that tooltips can explain why the action is unavailable.
