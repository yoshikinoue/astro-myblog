...
## 2026-02-18 - Disabled link accessibility
**Learning:** Using `pointer-events: none` on disabled links prevents native tooltips (`title` attribute) from displaying, making it difficult for users to understand why the link is disabled.
**Action:** Replace `pointer-events-none` with `cursor-not-allowed` on disabled elements. Ensure that semantically disabled `<a>` tags omit the `href` entirely (e.g., `href={undefined}`) to avoid unintentional click or scroll behavior while maintaining keyboard focusability for accessibility.
