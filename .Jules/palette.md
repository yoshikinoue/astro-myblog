...
## 2024-05-20 - [Accessibility] Fix Disabled Pagination Links Tooltips and Behavior
**Learning:** Using `pointer-events-none` on disabled elements (`.disabled` class in pagination) prevents native browser tooltips (`title` attribute) from displaying, robbing users of context. Similarly, setting `href="#"` on semantically disabled `<a>` tags causes accidental scroll-to-top behavior when clicked.
**Action:** Replace `pointer-events-none` with `cursor-not-allowed` on disabled elements to allow hover tooltips. Always omit the `href` attribute entirely (`href={undefined}`) for semantically disabled `<a>` tags.
