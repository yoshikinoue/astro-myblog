...
## 2026-03-22 - Improved disabled state visibility and semantics for `LinkButton`s
**Learning:** `LinkButton` had a few UX issues with disabled states. 1) Using `href="#"` for semantically disabled `<a>` tags causes page jump if accidentally clicked or interacted with via keyboard. 2) Adding `pointer-events-none` on disabled items prevents them from receiving hover events, making `title` tooltips inaccessible for visual mouse users attempting to understand why an action is disabled. 3) Missing `title` fallback on icon-only links if not explicitly passed, though they had an `ariaLabel`.
**Action:** 1) For semantically disabled links (`<a>` tags), omit the `href` attribute entirely (`href={undefined}`) instead of using a placeholder like `href="#"` to prevent accidental scroll-to-top behavior. 2) Avoid `pointer-events-none` on disabled interactive elements so that tooltips can display on hover; use `cursor-not-allowed` to give clear visual feedback. 3) Default `title` to `ariaLabel` on `LinkButton` if `title` is missing, to ensure both visual and screen reader users get the necessary context for icon-only buttons.
