...
## 2026-02-18 - Disabled Anchor Tags and Cursor State
**Learning:** Styling disabled `<a>` elements with `cursor-not-allowed` requires replacing `pointer-events: none` and simultaneously removing the `href` attribute (e.g., `href={undefined}`). Setting `href="#"` combined with `cursor-not-allowed` without `pointer-events: none` makes the link keyboard-focusable and clickable, violating accessibility standards. Removing the `href` entirely safely turns the `<a>` into a non-interactive element while preserving visual cues like tooltips and custom cursors.
**Action:** When styling disabled links to support tooltips or custom cursors (`cursor-not-allowed`), always remove the `href` attribute completely to prevent keyboard focus and click actions.
