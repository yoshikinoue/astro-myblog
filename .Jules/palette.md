...
## 2026-02-18 - Replacing pointer-events-none with cursor-not-allowed on disabled links
**Learning:** Using `pointer-events: none` on disabled elements prevents native browser tooltips (`title` attribute) from displaying on hover. Removing the `href` completely via `href={undefined}` while applying `cursor-not-allowed` ensures proper feedback while neutralizing click actions safely.
**Action:** When styling disabled links, use `cursor-not-allowed` instead of `pointer-events-none`, but make sure to also remove the `href` attribute.
