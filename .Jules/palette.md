...
## 2026-02-18 - Disabled Pagination Links Accessibility and UX
**Learning:** Using `pointer-events-none` on disabled pagination links prevents native browser tooltips (`title` attributes) from showing, leaving users without explanation for the disabled state. Additionally, using `href="#"` for disabled links can cause accidental scroll-to-top behavior when clicked if keyboard/screen-reader users focus them.
**Action:** When styling disabled links, use `cursor-not-allowed` rather than `pointer-events-none` and pass descriptive `title` attributes. For the underlying `<a>` tag of a semantically disabled link, omit the `href` attribute entirely (`href={undefined}`) instead of using a placeholder like `href="#"`.
