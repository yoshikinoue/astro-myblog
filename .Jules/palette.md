...
## 2026-02-18 - Improved Disabled State UX
**Learning:** Using `pointer-events-none` for disabled buttons prevents mouse events entirely, so the user never sees a `cursor-not-allowed` indicator.
**Action:** Instead of `pointer-events: none`, use `cursor-not-allowed` to provide visual feedback on hover, and explicitly remove the `href` attribute (`href={undefined}`) to prevent keyboard navigation or clicking, ensuring accessibility is maintained.
